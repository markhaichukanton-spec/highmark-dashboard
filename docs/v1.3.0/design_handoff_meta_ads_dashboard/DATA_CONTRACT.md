...
### Выбор строк ⇄ график ⇄ фильтры (важно для бэкенда)
У каждой строки таблицы (кампания/адсет/объявление) есть чекбокс. Отметка строки:
1. **скоупит график** — он показывает суммарный дневной ряд только выбранных
   сущностей (правило «самый детальный уровень побеждает»: если выбраны кампания
   и её объявление, считается только объявление — без двойного счёта);
2. **добавляет сущность в фильтр** соответствующего уровня (Campaign/Adset/Ad) —
   связь двунаправленная: выбор в выпадающем фильтре тоже отмечает строку.
...
замените на реальные ряды. Имена в `filters.Campaign/Adset/Ad` обязаны ТОЧНО
совпадать с `campaign`/`adset`/`ad` в таблице (по ним идёт сопоставление).
