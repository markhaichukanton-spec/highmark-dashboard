...
## 3. `geo` — разбивка по странам (донат)
...
Производные считаются так же. Строки с нулевым значением выбранной метрики
автоматически прячутся из доната (напр. Nigeria с revenue=0).
