...
### Выбор строк ⇄ график (важно для бэкенда)
У каждой строки таблицы (кампания/адсет/объявление) есть чекбокс. Выбор хранится
как множество **уникальных ключей по иерархии** (`campaign▸adset▸ad`, разделитель
`\u0001`) — поэтому отметка одного адсета НЕ выбирает одноимённые адсеты в других
кампаниях (это был баг). Отметка строки:
1. **скоупит график** — он показывает суммарный дневной ряд только выбранных
   строк (правило «самый детальный ключ побеждает»: если выбраны кампания и её
   объявление, считается только объявление — без двойного счёта);
2. **подсвечивает контекст** — таблица всегда показывает ВСЕ строки; выбранная
   строка + её предки (путь вверх) + её потомки остаются яркими, все прочие
   строки затемняются (класс `tr-dim`). Строки из таблицы не пропадают.

**Что нужно от бэкенда:** дневные ряды НЕ только суммарные, а **по каждой
сущности** — чтобы скоупить график по выбору. Идеально — эндпоинт
`GET /api/timeseries?level=campaign|adset|ad&id=...&from&to&granularity`,
возвращающий точки в формате секции `series` (сырые поля). В прототипе этих
рядов нет в источнике, поэтому фронт их **синтезирует** (раскладывает итог
каждой сущности по форме общего тренда и нормирует) — это заглушка, в проде
замените на реальные ряды. Сопоставление идёт по уникальному id/ключу строки,
а не по имени.
