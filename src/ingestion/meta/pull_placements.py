"""
pull_placements.py — Meta Marketing API: pull placement-level ad insights.

One row = date × publisher_platform × platform_position × campaign × adset × ad.
Writes to BigQuery raw_ad_placements (18 columns + _loaded_at).

Breakdowns: ["publisher_platform", "platform_position"]
Purchases and revenue ARE available at this level (confirmed via API test 2026-06-07).
Country is NOT available — Meta does not store country × placement as a joint breakdown.

Usage:
    python src/ingestion/meta/pull_placements.py --project aurora-scents --since 2026-01-01
    python src/ingestion/meta/pull_placements.py --project aurora-scents --since 2026-05-01 --until 2026-05-31
"""
...
def day_chunks(since: str, until: str, step: int = 3):
    """Yields (since, until) pairs in `step`-day windows."""
...
def pull_chunk(account: AdAccount, since: str, until: str) -> list:
    params = {
        ...
        "fields": FIELDS,
        "breakdowns": ["publisher_platform", "platform_position"],
        "limit": 1000,  # smaller pages — avoids "reduce data" error on heavy weeks
    }
...
def pull_all(account: AdAccount, since: str, until: str, account_id: str):
    print(f"Pulling {since} -> {until} (ad level + publisher_platform + platform_position)", flush=True)
    total = 0
...
def main() -> None:
    ...
    p.add_argument("--append", action="store_true",
                   help="Append to existing table instead of truncating (use when resuming)")
    args = p.parse_args()
    ...
    first_chunk = not args.append
    for _label, rows in pull_all(account, args.since, args.until, account_id):
        if rows:
            load_rows(bq, bq_project, bq_dataset, "ad_placements", rows, append=not first_chunk)
            first_chunk = False
